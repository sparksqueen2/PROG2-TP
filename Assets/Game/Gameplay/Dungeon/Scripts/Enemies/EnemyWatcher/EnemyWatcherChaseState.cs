...
public class EnemyWatcherChaseState : IEnemyState
{
    ...
    public void Execute()
    {
        ...
        else if (_controller.IsPlayerLost())
        {
            _enemyHealth?.DisableHealthBar();
            if (_controller.HasSufficientPatrolPoints())
                _controller.SetState(new EnemyWatcherPatrolState(_controller, _animation, _navigation));
            else
                _controller.SetState(new EnemyWatcherIdleState(_controller, _animation, _controller.IdleTimeout, _navigation));
        }
        else
        {
